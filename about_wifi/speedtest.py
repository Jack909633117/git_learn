...
def shell():
    ...
    logging.info('Retrieving speedtest.net configuration...')
    try:
        speedtest = Speedtest(
    ...
    printer('Testing from %(isp)s (%(ip)s)...' % speedtest.config['client'],
            quiet)
    logging.info('Testing from %(isp)s (%(ip)s)...' % speedtest.config['client'])

    if not args.mini:
        printer('Retrieving speedtest.net server list...', quiet)
        logging.info('Retrieving speedtest.net server list...')
        try:
        ...
        if args.server and len(args.server) == 1:
            ...
            logging.info('Selecting best server based on ping...')
    ...
    logging.info('Hosted by %(sponsor)s (%(name)s) [%(d)0.2f km]: '
    ...
    if args.download:
        ...
        printer('Download: %0.2f M%s/s' %
                ((results.download / 1000.0 / 1000.0) / args.units[1],
                 args.units[0]),
                quiet)
        logging.info('Download: %0.2f M%s/s' %
                ((results.download / 1000.0 / 1000.0) / args.units[1],
                 args.units[0]))
    else:
    ...
    if args.upload:
        ...
        printer('Upload: %0.2f M%s/s' %
                ((results.upload / 1000.0 / 1000.0) / args.units[1],
                 args.units[0]),
                quiet)
        logging.info('Upload: %0.2f M%s/s' %
                ((results.upload / 1000.0 / 1000.0) / args.units[1],
                 args.units[0]))
    else:
